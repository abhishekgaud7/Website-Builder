    ...
    <>
      <nav className="z-50 flex items-center justify-between w-full py-4 px-4 md:px-16 lg:px-24 xl:px-32 backdrop-blur border-b text-white border-slate-800">
        ...
        <div className="hidden md:flex items-center gap-8 transition duration-500">
          <Link to='/'>Home</Link>
          {session && <Link to='/projects'>My Projects</Link>}
          ...
          {session ? (
            <div className="flex items-center gap-3">
              ...
              <button
                ...
                className="px-4 py-1.5 max-sm:text-sm bg-slate-800 hover:bg-slate-700 active:scale-95 transition rounded-md border border-slate-700 cursor-pointer"
              >
                Sign Out
              </button>
            </div>
            ...
            <button
              ...
              className="px-6 py-1.5 max-sm:text-sm bg-indigo-600 active:scale-95 hover:bg-indigo-700 transition rounded-md cursor-pointer"
            >
              Get started
            </button>
          )}
      ...
      {menuOpen && (
        <div className="fixed inset-0 z-[100] bg-black/60 text-white backdrop-blur flex flex-col items-center justify-center text-lg gap-8 md:hidden transition-transform duration-300">
          ...
          {session && <Link to='/projects' onClick={() => setMenuOpen(false)}>My Projects</Link>}
      ...
      {/* BACKGROUND IMAGE */}
      <img src="https://raw.githubusercontent.com/prebuiltui/prebuiltui/refs/heads/main/assets/hero/bg-gradient-2.png" className="absolute inset-0 -z-10 size-full opacity-60 pointer-events-none" alt="" />
    </>
  )
}
